...
export default class BattleSystem {
  constructor(game, state, overworld, ambushState, playerSide) {
    ...
    this.overworldTile = overworld.getTile(this.playerSquad.x, this.playerSquad.y);

    // Create the local map
    game.playState.localMap = localMapCreation.createLocalMap(
      this.overworldTile,
      properties.localWidth,
      properties.localHeight
    );
    ...
    this.playerSquadLocalFov = new LocalFov(
      this.map,
      this.playerSquad.members,
      properties.fovConcealmentThreshold
    );
    this.enemySquadLocalFov = new LocalFov(
      this.map,
      this.enemySquad.members,
      properties.fovConcealmentThreshold
    );
    ...
    this.characters = squadProcedures.getAllMembersByTurnOrder(this.playerSquad, this.enemySquad);
  ...
  nextCharacter() {
    ...
    const enemyEnvironmentDamageActions = this.environmentSystem.damageCharacters(
      this.ballisticsSystem,
      this.enemySquad
    );
    const playerEnvironmentDamageActions = this.environmentSystem.damageCharacters(
      this.ballisticsSystem,
      this.playerSquad
    );
  ...
  handleInput(input, local) {
    ...
    switch (input) {
      case 'LEFT':
        if (this.targetMode) {
          this.target.x = utils.clamp(this.target.x - 1, 0, local.width - 1);
          this.setTarget();
        }
        else {
          ...
          this.currentCharacter.x = utils.clamp(x - 1, 0, local.width - 1);
          this.currentCharacterMoves--;
      ...
      case 'UP':
        if (this.targetMode) {
          this.target.y = utils.clamp(this.target.y - 1, 0, local.height - 1);
          this.setTarget();
        }
        else {
          const { x, y } = this.currentCharacter;
          ...
          this.currentCharacter.y = utils.clamp(y - 1, 0, local.height - 1);
          this.currentCharacterMoves--;
          this.characterIsMoving = true;
          this.playerSquadLocalFov.recalculate();
        }
      ...
      case 'WAIT':
        ...
        this.messages.push({ name: this.currentCharacter.name, text: 'waits' });
        break;
      case 'ATTACK':
        ...
        if (!this.characterIsMoving) {
          ...
          const primaryAmmoCount =
            this.playerSquad.inventory.getAmmoCountForWeapon(this.currentCharacter.weapon) || 0;
          const secondaryAmmoCount =
            this.playerSquad.inventory.getAmmoCountForWeapon(this.currentCharacter.secondary) || 0;
      ...
      case 'TOGGLE WEAPON': {
        ...
        if (
          this.currentCharacter.primarySelected &&
          this.currentCharacter.hasSecondaryWeapon() &&
  ...
  shouldMove(local, nextX, nextY) {
    ...
    if (
      ...
      const attackActions = this.ballisticsSystem.effectMelee(
        this.currentCharacter,
        defendingCharacter
      );
      ...
      text.createBattleMessages(attackActions).forEach(message => this.messages.push(message));
  ...
  moveAnimationFrame() {
    ...
    if (this.movement.index >= this.movement.line.length) {
  ...
  shouldFireProjectile(target) {
    ...
    const { effectAreas, smokeAreas, fireAreas, attackActions } = this.ballisticsSystem.effectFire(
      this.currentCharacter,
      firedWeapon,
      this.projectile.intendedLine,
      expendedAmmo
    );
    ...
    this.projectile.fireAnimation = this.ballisticsSystem.generateFireAnimation(
      firedWeapon,
      this.projectile.effectAreas,
      expendedAmmo
    );
  ...
  fireAnimationFrame() {
    ...
    if (fireAnimation.fireSequenceIndex >= fireAnimation.fireSequence.length) {
      ...
      this.projectile.smokeAreas.forEach(point =>
        this.environmentSystem.addSmoke(point, point.amount)
      );
      this.projectile.fireAreas.forEach(point =>
        this.environmentSystem.addFire(point, point.amount)
      );
  ...
  targetClosestEnemy() {
    ...
    const enemyNumber = closestEnemies.length > 0 ? closestEnemies[0].number : null;
  ...
  targetNextEnemy() {
    ...
    const currentTargetedNumber = this.target.enemy ? this.target.enemy.number : -1;
  ...
  squadOnBorder(local) {
    const someNotOnBorder = this.playerSquad.getAliveMembers().some(member => {
      // Not on the border
      if (
        member.x !== 0 &&
        member.x !== local.width - 1 &&
        member.y !== 0 &&
        member.y !== local.height - 1
      ) {
        console.log(member);
        return true;
      }
  ...
  endBattle(outcome) {
    ...
    if (outcome.win) {
      ...
      const loot = squadProcedures.getLootByEnemySquad(
        this.enemySquad.members,
        this.enemySquad.inventory
      );
